...
from vidore3_ablations.data import AVAILABLE_DATASETS, load_vidore_dataset
from vidore3_ablations.fiftyone_utils import boxes_to_fiftyone_detections
...
def build_fiftyone_dataset(
    ...
    if max_corpus is not None:
        ...
        for qid in query_ids:
            needed.update(qrels.get(qid, {}).keys())
        ...
        for cid in corpus_ids:
            ...
            if len(keep_order) >= max_corpus:
                break
        keep = set(keep_order[:max_corpus])
        ...
        corpus_ids, corpus_images, corpus_texts = filtered_ids, filtered_images, filtered_texts
    ...
    # Load full qrel metadata (content_type) from HuggingFace
    ...
    dataset.description = f"ViDoRe v3 explorer ({view}) for {dataset_name}"
...
def _build_corpus_view(
    ...
    # Aggregate all boxes per corpus page across queries
...
def _build_queries_view(
    ...
    for query_id, query_text in tqdm(zip(query_ids, queries), total=len(query_ids), desc="Building queries view"):
        ...
        # Use highest-relevance page as thumbnail
